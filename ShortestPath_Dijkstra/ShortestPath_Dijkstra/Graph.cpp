...
int ** MakeRandom_Adjacency(int **Matrix,int nodeNum)
{
	...
	for (int i = 0;i <nodeNum;i++)//loop for creating upper triangle
	{
		for (int j = i + 1; j < nodeNum; j++)
		{
			Matrix[i][j] = Matrix[j][i];//creating symmetric matrix
		}
	}
	return Matrix;//returns matrix
}
...
void Print_Adjacency(int**Matrix,int nodeNum)
{
	...
	cout << "  ";
	for (int i = 0;i < nodeNum;i++)//printing order for column
		cout << setw(4) << i;
	cout << endl << "  ";
...
void Find_Information(int ** Matrix, int nodeNum)
{
	...
	{
		cout << "DISTANCE BETWEEN " << NormalOutput[i] << "-" << NormalOutput[i - 1] << " = " << Matrix[NormalOutput[i]][NormalOutput[i - 1]] << endl;
	}
